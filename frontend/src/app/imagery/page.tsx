...
export default function ImageryWorkspace() {
  ...
  const handleFarmChange = async (farmId: string) => {
    ...
    // Reload imagery for the new farm, or all if farmId is empty
    loadImagery(farmId);
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await loadImagery(selectedFarm);
    } catch (e: unknown) {
  ...
  return (
    <div className="p-8 w-full space-y-8 text-black">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        ...
        <form onSubmit={handleUpload} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div>
            ...
            <select 
              className="w-full border p-2 rounded-md disabled:bg-gray-100"
              value={selectedFarm}
              onChange={(e) => handleFarmChange(e.target.value)}
              disabled={isFarmsLoading}
            >
              <option value="">{isFarmsLoading ? "Loading farms..." : farms.length === 0 ? "No farms available" : "Select Farm..."}</option>
              {farms.map(f => (
          ...
          <div>
            ...
            <select 
              ...
              disabled={!selectedFarm || isPondsLoading}
            >
              <option value="">{isPondsLoading ? "Loading ponds..." : ponds.length === 0 && selectedFarm ? "No ponds available" : "Select Pond..."}</option>
              {ponds.map(p => (
      ...
      {/* List Section */}
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <h2 className="text-xl font-semibold mb-4">Ingested Imagery {selectedFarm && farms.find(f => f.id === selectedFarm)?.name ? `(${farms.find(f => f.id === selectedFarm)?.name})` : ""}</h2>
        {isImageryLoading ? (
